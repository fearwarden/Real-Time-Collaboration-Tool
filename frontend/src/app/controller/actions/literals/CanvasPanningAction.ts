...
export default class CanvasPanningAction extends AbstractMouseAction {
    ...
    onMouseMove(e: MouseEvent): void {
        ...
        canvas.cameraOffset = { x: Math.min(canvas.cameraOffset.x, canvas.width * 1.5), y: Math.min(canvas.cameraOffset.y, canvas.height * 1.5) }
        canvas.cameraOffset = { x: Math.max(canvas.cameraOffset.x, -canvas.width * 1.5), y: Math.max(canvas.cameraOffset.y, -canvas.height * 1.5) }
    }
    ...
    onMouseDown(e: MouseEvent): void {
        const canvas = Main.getInstance().canvas!;
        const mousePosition = CoordinateUtils.getCanvasMousePosition(e);
        this.panningStart = { x: mousePosition.x - canvas.cameraOffset.x, y: mousePosition.y - canvas.cameraOffset.y };
        this.isPanning = true;
    }
