...
export default class EraserState extends AbstractDrawingState {
	...
	public onMouseMove(event: MouseEvent): void {
		...
		const intersectedLineIndex = this.findIntersectedLine(screenToWorld.x, screenToWorld.y);

		// If a line is intersected, delete it
		if (intersectedLineIndex !== -1) {
			Main.getInstance().strategyModel.drawingPoints.splice(intersectedLineIndex, 1);
		}
	}
	...
	private findIntersectedLine(mouseX: number, mouseY: number): number {
		...
		for (let lineIndex = 0; lineIndex < drawingPoints.length; lineIndex++) {
			...
			for (let i = 0; i < line.length - 1; i++) {
				...
				if (
					CoordinateUtils.pointToSegmentDistance(mouseX, mouseY, x1, y1, x2, y2) <=
					this.thickness
				) {
					return lineIndex;
				}
